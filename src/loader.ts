...
const loadRemoteWidgets = async () => {
  for (const repo of repositoryConfig) {
    fetch(repo.url + "/widets.json")
      .then((res) => res.json())
      .then((data) => {
        for (const widgetName of data.widgets) {
          const propertyPromise = loadComponent(
            repo.name,
            repo.url,
            widgetName
          );
